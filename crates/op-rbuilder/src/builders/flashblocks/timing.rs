...
mod tests {
    ...
    fn send_time_intervals(scheduler: &FlashblockScheduler) -> Vec<Duration> {
        scheduler
            .send_times
            .iter()
            .map(|t| *t - scheduler.reference_instant)
            .collect()
    }
    ...
    async fn test_scheduler_run_block_cancellation() {
        ...
        assert_eq!(
            count, 4,
            "Expected 1 immediate + 3 scheduled triggers before cancel"
        );
    }
    ...
    async fn test_scheduler_run_cancellation_token_lifecycle() {
        ...
        assert!(
            ...
            "Immediate token should be cancelled by first trigger"
        );
        assert!(
            tokens[1].is_cancelled(),
            "First trigger token should be cancelled by second"
        );
        assert!(
            tokens[2].is_cancelled(),
            "Second trigger token should be cancelled by third"
        );
        assert!(
            tokens[3].is_cancelled(),
            "Third trigger token should be cancelled by fourth"
        );
        assert!(
            tokens[4].is_cancelled(),
            "Fourth trigger token should be cancelled by fifth"
        );
        assert!(
    ...
    async fn test_scheduler_run_late_fcu_backwards_compat() {
        ...
        assert_eq!(
            count, 6,
            "Expected 1 immediate + 5 scheduled triggers even with late FCU"
        );
    }
}
